...
func _award_items(items: Array) -> void:
	for item_id in items:
		...
		var award_position = digging_area.global_position if digging_area else global_position
		GameState.collect_item_at_position(item_id, 1, award_position)
		...
		if Engine.get_main_loop().root.has_node("VisualFeedbackController"):
			...
			if feedback.has_method("item_pickup_effect"):
				...
				feedback.item_pickup_effect(award_position, item_texture)

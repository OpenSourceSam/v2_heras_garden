...
const DEFAULT_SETTINGS = {
	...
	# Default to the gdtoolkit binary (installed in PATH) so the formatter works out of the box.
	SETTING_FORMATTER_PATH: "gdformat",
...
func _init() -> void:
	...
	# Existing installs may have an empty formatter path; ensure we default to the gdformat binary.
	if str(get_editor_setting(SETTING_FORMATTER_PATH)).strip_edges() == "":

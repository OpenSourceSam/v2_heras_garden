...
enum BackendType { OLLAMA, OPENAI, CLAUDE, DOCKER, GEMINI, MINIMAX }
...
func set_backend(backend_type: BackendType, api_key: String = "") -> void:
	match backend_type:
		...
		BackendType.MINIMAX:
			current_backend = MinimaxBackend.new()
...
func _get_connection_error_message(provider_name: String) -> String:
	...
	match provider_name:
		...
		"MiniMax":
			msg += "Can't connect to MiniMax API.\n\n"
			msg += "To fix this:\n"
			msg += "1. Check your internet connection\n"
			msg += "2. Verify your API key from: platform.minimaxi.com\n"
			msg += "3. Ensure your API key has proper permissions"
		_:
